...
_Source: `server.js:L1`, `server.js:L3-L4`._
...
| **Unusual / non-standard methods** | `PATCH`, `OPTIONS`, `HEAD`, or a custom verb | `200`, `text/plain`, body `Hello, World!\n` for body-bearing methods (e.g. `PATCH`, `OPTIONS`). For `HEAD`, Node follows HTTP semantics: status `200` and `Content-Type: text/plain` still match, but **no response body is sent and `Content-Length` is omitted** | `server.js:L6-L10` |
| **Request with a body** | `POST`/`PUT` with a payload | Body is read but **ignored**; response is still `Hello, World!\n` | `server.js:L6-L10` |
...
`node --experimental-test-coverage` (no install), or third-party tools such as
[`c8`](https://www.npmjs.com/package/c8) or [`nyc`](https://www.npmjs.com/package/nyc) can be
used. **Adopting any of these would add a devDependency, which is out of scope for this
project today** — they are mentioned for completeness only.
...
| **P2** | **Edge cases** (long/encoded paths, bodies ignored, concurrency, idempotency) | Medium — robustness and determinism under varied input/load | Subtle non-determinism | Valuable but lower-yield given the branchless handler. _Source: `server.js:L6-L10`._ |
...
- **`node:test` + global `fetch`** — almost zero-dependency; only a thin convenience layer
  over what Node already provides.
