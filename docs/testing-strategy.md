...
const get = (url) => new Promise((resolve, reject) => {
  // Fresh connection per request (agent: false) keeps each probe independent and deterministic.
...
> **Testing note — bodies don't change the response; probe over a fresh connection for deterministic, isolated requests.** `server.js` calls `res.end()` without reading the request stream (`req`) and answers **every** request identically regardless of method, path, or body. A controlled probe of the running server — `POST` / `PUT` / `DELETE` carrying bodies from 16 bytes up to several megabytes, plus body-less `GET` / `OPTIONS`, across both fresh (`agent: false`) and reused keep-alive connections (well over 1,000 requests in total) — confirmed the application response is **invariant** on the verified **Node v20.20.2** runtime: HTTP `200`, `Content-Type: text/plain`, body `Hello, World!\n` (14 bytes), with **zero** incorrect responses and **zero** connection resets. The server keeps connections alive (it sends `Connection: keep-alive` to keep-alive clients). One transport nuance worth designing around defensively: because the handler never consumes the request body, clean reuse of a keep-alive socket relies on the runtime draining that unread body before the next request — edge-case behavior that is implementation-dependent across Node versions. Issuing each request on a **fresh connection** — `agent: false`, as the snippets above do, or a `Connection: close` request header — removes that dependency entirely and keeps every probe independent. **P0 takeaway:** vary method, path, and body freely and assert the **same** application response (`200` / `text/plain` / `Hello, World!\n`), probing over fresh connections so each request stays deterministic and isolated regardless of Node version.
